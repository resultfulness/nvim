return {
  ...
  opts = {},
  config = function ()
    vim.keymap.set({'n', 'x'}, '<leader>rr', require('telescope').extensions.refactoring.refactors, { desc = '[R]efactor: Select [R]efactor From List' })

    local function rmap(mode, key_suffix, refactor_name, desc)
      vim.keymap.set(mode, '<leader>r' .. key_suffix, function() require('refactoring').refactor(refactor_name) end, { desc = '[R]efactor: ' .. desc })
    end
